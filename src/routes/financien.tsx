...
function FinancienPage() {
  ...
  const byCategory = [...categoryMap.entries()].sort(
    ...
    <div className="max-w-3xl mx-auto px-5 md:px-8 py-8 md:py-12 space-y-10">
        ...
        <div className="space-y-2">
          {(accounts.data ?? []).map((a: any) => (
            <AccountRow
              key={a.id}
              account={a}
              onChanged={() => qc.invalidateQueries({ queryKey: ["accounts-balance"] })}
            />
          ))}
        </div>
      </section>
      ...
      <section>
        ...
        <p className="text-xs text-muted-foreground mb-3 px-1">
          Per kwartaal/jaar wordt omgerekend naar maandelijks equivalent.
        </p>
            ...
            <div key={scope} className="mb-5">
              <div className="flex items-baseline justify-between mb-2 px-1">
                <p className="text-xs uppercase tracking-wider text-muted-foreground">
                  {scope === "prive" ? "Privé" : "Zakelijk"}
                </p>
                <p className="text-xs tabular-nums text-muted-foreground">{fmt(subtotal)}/maand</p>
              </div>
              <div className="space-y-2">
                {items.length === 0 && (
                  <p className="text-xs text-muted-foreground px-1 italic">Geen kosten</p>
                )}
                {items.map((r: any) => {
                    ...
                    <div key={r.id} className="glass-card rounded-xl p-4 flex items-center gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {freq === "monthly" &&
                            `Elke ${r.day_of_month}${ord(r.day_of_month)} v/d maand`}
                          {freq === "quarterly" && `Per kwartaal · dag ${r.day_of_month}`}
                          {freq === "biannual" && `2× per jaar · dag ${r.day_of_month}`}
                          {freq === "yearly" &&
                            `Jaarlijks${r.month_of_year ? ` · ${monthName(r.month_of_year)}` : ""}`}
        ...
        <AddRecurring
          onAdded={() => qc.invalidateQueries({ queryKey: ["recurring"] })}
          userId={user?.id}
        />
      </section>
        ...
        <div className="space-y-2">
          {(deposits.data ?? []).map((d: any) => (
            <div key={d.id} className="glass-card rounded-xl p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                ...
                {d.counterparty && (
                  <p className="text-xs text-muted-foreground">bij {d.counterparty}</p>
                )}
              </div>
          ...
          <div className="glass-card rounded-xl p-4 flex items-center gap-4 opacity-80">
            <div className="flex-1">
              <p className="text-sm text-muted-foreground">Totaal vastgezet</p>
            </div>
            <p className="tabular-nums font-semibold">{fmt(totalDeposits)}</p>
          </div>
        </div>
        <AddDeposit
          onAdded={() => qc.invalidateQueries({ queryKey: ["deposits"] })}
          userId={user?.id}
        />
      </section>
      ...
      <section>
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Op basis van betaalde facturen, automatisch gecategoriseerd door AI en omgerekend naar
          EUR.
        </p>
        {byCategory.length === 0 ? (
                ...
                <div key={cat} className="glass-card rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {v.count} {v.count === 1 ? "factuur" : "facturen"}
                      </p>
                    </div>
...
function AddRecurring({ onAdded, userId }: { onAdded: () => void; userId?: string }) {
  ...
  const [frequency, setFrequency] = useState<"monthly" | "quarterly" | "biannual" | "yearly">(
    "monthly",
  );
  ...
  return (
    <form
      ...
      className="glass-card rounded-xl p-4 mt-3 space-y-3"
    >
