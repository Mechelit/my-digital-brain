  ...
  return (
    <div className="max-w-3xl mx-auto px-5 md:px-8 py-8 md:py-12">
      <header className="mb-10">
        <p className="text-sm text-muted-foreground">{now.toLocaleDateString("nl-BE", { weekday: "long", day: "numeric", month: "long" })}</p>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to="/scan">
          <Button size="sm" variant="secondary"><Camera className="w-4 h-4 mr-2" />Scan</Button>
        </Link>
      </div>
      ...
      {pending.length === 0 ? (
        <div className="glass-card rounded-2xl p-10 text-center">
          ...
          <Link to="/scan"><Button className="mt-5">Eerste factuur scannen</Button></Link>
        </div>
      ) : (
      ...
      {paid.length > 0 && (
        <>
          ...
          <div className="space-y-2">{paid.slice(0, 5).map((inv) => <InvoiceCard key={inv.id} invoice={inv} />)}</div>
        </>
      )}
    </div>
  );
